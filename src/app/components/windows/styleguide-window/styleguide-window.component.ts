...
import { ThemeService, ThemeId, THEME_REGISTRY } from '../../../services/general/theme.service';
...
@Component({
  selector: 'app-styleguide-window',
  imports: [CommonModule, DraggableDirective, PixelIconComponent, FormsModule],
  templateUrl: 'styleguide-window.component.html',
  styleUrls: ['styleguide-window.component.scss']
})
export class StyleguideWindowComponent extends FloatWindow {
  activeSection = signal<string>('buttons');
  iconSearch = signal('');

  sections: Section[] = [
    { id: 'buttons', label: 'Buttons', icon: 'gamepad' },
    { id: 'forms', label: 'Forms', icon: 'edit' },
    { id: 'containers', label: 'Containers', icon: 'folder' },
    { id: 'tables', label: 'Tables', icon: 'view-list' },
    { id: 'icons', label: 'Icons', icon: 'grid' },
    { id: 'themes', label: 'Themes', icon: 'sliders' },
    { id: 'typography', label: 'Type', icon: 'article' }
  ];
  ...
  // Use the theme registry from the service
