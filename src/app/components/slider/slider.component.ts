...
export class CvdSliderComponent implements OnInit {
  ...
  percentage: WritableSignal<number> = signal<number>(0);
  ...
  ngOnInit() {
    ...
    this.percentage.set((this.value() - this.min()) / (this.max() - this.min()) * 100);
  }

  onElementInput(event: Event) {
    const target = event.target as HTMLInputElement;
    this.percentage.set(Number(target.value));
